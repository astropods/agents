...
describe("deduplicate", () => {
  test("returns all articles when titles are unique", () => {
    const articles = [
      makeArticle("Article A"),
      makeArticle("Article B"),
      makeArticle("Article C"),
    ];
...
describe("detectFormat", () => {
  ...
  test("strips format keyword from topic", () => {
    expect(detectFormat("startup funding analysis").topic).toBe(
      "startup funding",
    );
  ...
  test('strips "analyse" and "analyze" from topic', () => {
    ...
    expect(detectFormat("analyze quantum computing").topic).toBe(
      "quantum computing",
    );
  });
