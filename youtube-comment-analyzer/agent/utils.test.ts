...
describe("parseBatchResults", () => {
  test("parses a plain JSON response", () => {
    const raw = JSON.stringify({
      results: [
        { sentiment: "positive", needs_reply: false },
        { sentiment: "neutral", needs_reply: true },
        { sentiment: "negative", needs_reply: true },
      ],
    });
    expect(parseBatchResults(raw)).toEqual([
      { sentiment: "positive", needsReply: false },
      { sentiment: "neutral", needsReply: true },
      { sentiment: "negative", needsReply: true },
    ]);
  });

  test("strips markdown code fences", () => {
    const raw =
      '```json\n{"results":[{"sentiment":"positive","needs_reply":false}]}\n```';
    expect(parseBatchResults(raw)).toEqual([
      { sentiment: "positive", needsReply: false },
    ]);
  });

  test("normalises unknown sentiment values to neutral", () => {
    const raw = JSON.stringify({
      results: [{ sentiment: "angry", needs_reply: true }],
    });
    expect(parseBatchResults(raw)).toEqual([
      { sentiment: "neutral", needsReply: true },
    ]);
  ...
  test("needs_reply defaults to false when not true", () => {
    const raw = JSON.stringify({
      results: [{ sentiment: "positive", needs_reply: "yes" }],
    });
    expect(parseBatchResults(raw)).toEqual([
      { sentiment: "positive", needsReply: false },
    ]);
  ...
  test("throws on invalid JSON", () => {
    expect(() => parseBatchResults("not json")).toThrow();
  });
});
...
function makeResult(
  ...
  return { comment, sentiment, needsReply };
}

describe("formatReport", () => {
  test("includes video URL in output", () => {
    const report = formatReport(
      [makeResult("great", "positive")],
      "dQw4w9WgXcQ",
    );
  ...
  test("shows correct needs reply count", () => {
    const results = [
      makeResult("a", "positive", true),
      makeResult("b", "positive", false),
      makeResult("c", "negative", true),
    ];
    const report = formatReport(results, "abc");
    expect(report).toContain("NEEDS REPLY  2 comments");
  });

  test("shows up to 5 needs reply examples", () => {
    const results = Array.from({ length: 7 }, (_, i) =>
      makeResult(`comment ${i}`, "neutral", true),
    );
    const report = formatReport(results, "abc");
    const matches = report.match(/! "/g) ?? [];
  ...
  test("shows 0 needs reply when no comments need reply", () => {
    const results = [
      makeResult("a", "positive", false),
      makeResult("b", "neutral", false),
    ];
    const report = formatReport(results, "abc");
