...
export class SalesforceClient {
  ...
  private validateQueryIntent(queryIntent: string, intentType: 'account' | 'opportunity'): string {
    const valid =
      intentType === 'account'
  ...
  private async getAccessToken(): Promise<string | null> {
    ...
    if (!this.clientId || !this.clientSecret || !this.refreshToken) {
      console.error(
        'Salesforce credentials not configured (SF_CLIENT_ID, SF_CLIENT_SECRET, SF_REFRESH_TOKEN)',
      );
      return null;
    }
    ...
    try {
      ...
      if (!res.ok) {
        console.error(
          `Salesforce OAuth failed (${res.status}): ${JSON.stringify(data).slice(0, 200)}`,
        );
        return null;
      }
  ...
  private sanitizeSoslSearchTerm(searchTerm: string): string {
    ...
    while (sanitized.includes('  ')) sanitized = sanitized.replace(/ {2}/g, ' ');
    return sanitized.trim();
  }

  async searchAccounts(searchTerm: string, maxMatches = 10): Promise<RecordLike[]> {
    const token = await this.getAccessToken();
    if (!token) return [];
  ...
  private filterAccountDataByIntent(accountData: RecordLike, queryIntent: string): RecordLike {
  ...
  private transformAccountData(account: RecordLike): RecordLike {
    ...
    const num = (k: string): number | null => SalesforceClient.parseNum(get(k));
    ...
    return {
      ...
      business_info: {
        ...
        employee_count: num('Employee_Count__c') ?? num('NumberOfEmployees'),
      ...
      usage_context: {
        use_cases: useCases,
        current_state: SalesforceClient.cleanHtml(get('Current_State_Prolifiq__c') as string),
        desired_state: SalesforceClient.cleanHtml(get('Desired_State_Prolifiq__c') as string),
      },
      engagement_metrics: {
        engagement_minutes_7d: num('engagio__EngagementMinutesLast7Days__c'),
        engagement_minutes_30d: num('engagio__EngagementMinutesLast30Days__c'),
        engagement_minutes_90d: num('engagio__EngagementMinutesLast3Months__c'),
        ...
        paid_session_hours_last_month: num('Paid_Session_Time_Last_Month_in_Hrs_Acc__c'),
      },
      product_info: {
        ...
        child_account_paid_licenses: num('Child_Account_Paid_LicensesDLRS__c'),
      },
      marketing_context: {
  ...
  async lookupAccount(
    ...
    const intent = this.validateQueryIntent(queryIntent, 'account') as AccountIntent;

    console.log(`Salesforce account lookup: '${accountNameFromQuery}' (intent: ${intent})`);
    ...
    if (searchResults.length === 0) {
      ...
      for (const char of specialChars) {
        if (accountNameFromQuery.includes(char)) {
          ...
          console.log(`No results for '${accountNameFromQuery}', trying '${variation}'`);
    ...
    const accountNames = searchResults.map((r) => (r.Name as string) ?? '');
    ...
    if (!account) {
      console.error(`Failed to get full data for ${primaryAccountId}, using basic data`);
      return {
    ...
    const primaryAccountContext = this.filterAccountDataByIntent(primaryAccountContextFull, intent);
  ...
  async getOpportunityFullData(opportunityId: string): Promise<RecordLike | null> {
  ...
  async getOpportunityContactRoles(opportunityId: string): Promise<RecordLike[]> {
    const token = await this.getAccessToken();
    if (!token) return [];
  ...
  private filterOpportunityDataByIntent(
    ...
    if (queryIntent === 'loss') {
      return {
        ...
        loss_reason_further_details_who: opportunityData.loss_reason_further_details_who,
        loss_reason_further_details_why: opportunityData.loss_reason_further_details_why,
    ...
    if (queryIntent === 'deal_details') {
      return {
        ...
        sso_identity_access_provider: opportunityData.sso_identity_access_provider,
  ...
  private transformOpportunityData(opportunity: RecordLike): RecordLike {
    ...
    return {
      ...
      loss_reason_further_details_who: get('Loss_Reason_Further_Details_Who__c'),
      loss_reason_further_details_why: get('Loss_Reason_Further_Details_Why__c'),
      ...
      was_sso_determining_factor: get('Was_SSO_a_determining_factor_for_Enterpr__c'),
      ...
      commissionable_arr_adjustment_date: get('Commissionable_ARR_Adjustment_Date__c'),
      current_team_arr: num('Current_Team_ARR__c'),
      net_incremental_arr: num('Net_Incremental_ARR__c'),
      adjustments_to_net_incremental_arr: num('Adjustments_to_Net_incremental_ARR__c'),
      adjustment_to_commissionable_arr: num('Adjustment_to_Commissionable_ARR__c'),
      ...
      total_team_arr_at_booking: num('Total_Team_ARR_at_Booking__c'),
      record_metadata: {
        created_date: get('CreatedDate'),
        last_modified_date: get('LastModifiedDate'),
        created_by_id: get('CreatedById'),
        ...
        is_excluded_from_territory2_filter: get('IsExcludedFromTerritory2Filter'),
      },
    };
  }
  ...
  async lookupOpportunity(
    ...
    const intent = this.validateQueryIntent(queryIntent, 'opportunity') as OpportunityIntent;
    ...
    if (opportunityId) {
      ...
      if (includeContactRoles) {
        const ocrRecords = await this.getOpportunityContactRoles(opportunity.Id as string);
        contactRoles = ocrRecords.map((ocr) => this.transformOpportunityContactRoleData(ocr));
      }
      return {
    ...
    if (accountId) {
      ...
      console.log(`Retrieved ${opportunities.length} opportunities for account ${accountId}`);
