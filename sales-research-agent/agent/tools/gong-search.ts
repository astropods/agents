...
If user provides a Gong call URL (https://gong.io/call?id=123), extract call_id and pass it.`,
  inputSchema: z.object({
    query: z
      .string()
      .describe('Natural language search query (include participant names if relevant)'),
  ...
  execute: async (input) => {
    ...
    console.log(
      `  [gongSearch] query="${input.query.slice(0, 80)}" account="${input.accountName}" callId="${callId}"`,
    );
    ...
    try {
      ...
      if (input.fromDate) {
        ...
        filter.call_date_unix = {
          ...((filter.call_date_unix as Record<string, number>) || {}),
          $gte: fromUnix,
        };
      }
      if (input.toDate) {
      ...
      const sources = matches.map((m) => {
        ...
        return {
          ...
          speaker:
            typeof md.speaker === 'object'
              ? ((md.speaker as Record<string, string>)?.name ?? 'Unknown')
              : String(md.speaker ?? 'Unknown'),
          text: String(md.text ?? ''),
